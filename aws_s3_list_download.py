...
SECRET_KEY='XXXXXXYYYYYZZZZZZZZZZZZZXXXXX'
args = parser.parse_args()
if args.list:
    session = Session(aws_access_key_id=ACCESS_KEY,aws_secret_access_key=SECRET_KEY)
    s3 = session.resource('s3')
    your_bucket = s3.Bucket('XXXYYSSSDDD')
    for s3_file in your_bucket.objects.all():
        print(s3_file.key)

if args.download:
    file = input('File to download specify with "XXX": ')
    session = Session(aws_access_key_id=ACCESS_KEY, aws_secret_access_key=SECRET_KEY)
    s3 = session.resource('s3')
    your_bucket = s3.Bucket('XXXXXYYYYSSSSDDD')
    for s3_file in your_bucket.objects.all():
        your_bucket.download_file('str(file)','str(file)')
